...
- (id)performTarget:(NSString *)targetName action:(NSString *)actionName params:(NSDictionary *)params shouldCacheTarget:(BOOL)shouldCacheTarget
{
    ...
    if (swiftModuleName.length > 0) {
        targetClassString = [NSString stringWithFormat:@"%@.Target%@", swiftModuleName, targetName];
    } else {
...
{
    SEL action = NSSelectorFromString(@"actionresponse:");
    NSObject *target = [[NSClassFromString(@"TargetNoTargetAction") alloc] init];
